...
const MyProfile = () => {
  ...
  const [userData, setUserData] = useState({
    ...
    email: 'JamesAdvert@gmail.com',
    phone: '+1 123 456 7890',
    address: {
      line1 :'1234 Main Street, NewYork, NY 10001',
      line2 :'Apt 1234, 5678 Main Street, NewYork, NY 10001'
    },
    gender: 'Male',
    dob: '01/01/1990'  
  ...
  return (
    <div className='max-w-lg flex flex-col gap-4 text-sm'>
      ...
      <div>
        <p className='text-neutral-500 unserline mt-3'>CONTACT INFORMATION</p>
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neautal-700'>
          <p className='font-medium'>Email id:</p>
          <p className='text-blue-500'>{userData.email}</p>
          <p className='font-medium'>Phone :</p>
          {
            editMode ? <input className='bg-gray-100 max-w-52' type="text" value={userData.phone} onChange={(e) => setUserData(prev => ({...prev, phone:e.target.value}))} /> : <p className='text-blue-400'>{userData.phone}</p>
          }
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>  
          ...
          {
            editMode? <input className='max-w-28 bg-gray-100' type='date' value={userData.dob} onChange={(e) => setUserData(prev => ({...prev, dob: e.target.value}))}/> : <p className='text-gray-400'>{userData.dob}</p>
          }
        </div>
      </div>
